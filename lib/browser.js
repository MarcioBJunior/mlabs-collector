...
export async function getBrowser() {
  ...
  const headless = process.env.BROWSER_HEADLESS === 'new' ? 'new' : true;
  const timeout = parseInt(process.env.BROWSER_TIMEOUT) || 30000;

  try {
    browser = await puppeteer.launch({
      headless: 'new',
      executablePath: await puppeteer.executablePath(), // Volta a usar Chrome baixado
      args: [
        '--no-sandbox',
      ...
      timeout,
      dumpio: false // Desabilita logs detalhados para reduzir ruído
    });
